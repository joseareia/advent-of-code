...
sub get_complexity { 
    my ($codes, $total, $complexity) = (shift, 0, 0);
    my @keypads = (get_numeric_keypad(), get_directional_keypad(), get_directional_keypad());

    foreach my $code (@$codes) {
        my $count = get_minimum_move_count($code, \@keypads, 0);
...
sub get_minimum_move_count {
    my ($buttons, $keypads, $pad, $count) = (shift, shift, shift, 0);
    ...
    foreach my $button (@$buttons) {
        ...
        foreach my $path (@$paths) {
            ...
            my $value;
            if (@$keypads > $pad + 1) {
                $value = get_minimum_move_count($path, $keypads, $pad + 1);
            } else {
                $value = scalar(@$path);
            }
