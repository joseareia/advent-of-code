...
-- Creates the Python script file and places it inside the previously created directory.
utils.show_progress("Creating the script files")
for i = 1, 2 do
    local filename = folder .. "/" .. utils.pad_number(i) .. ".py"
    local file = io.open(filename, "w")
    if not file then print(nok .. "Failed to create the script files.") goto exit end
    script_content = utils.script(i)
    file:write(script_content) -- Content that will be placed inside the script files.
    file:close()
end
