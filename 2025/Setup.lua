...
-- Creates the Lua script file and places it inside the previously created directory.
utils.show_progress("Creating the script files")
for i = 1, 2 do
    local lua_filename = folder .. "/" .. utils.pad_number(i) .. ".lua"
    local lua_file = io.open(lua_filename, "w")
    if not lua_file then print(nok .. "Failed to create the script files.") goto exit end
    local script_content = utils.lua_script(i)
    lua_file:write(script_content) -- Content that will be placed inside the script files.
    lua_file:close()
end
